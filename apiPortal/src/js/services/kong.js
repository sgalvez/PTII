        ...
        var factory = {
            ...
            handleError: function (response, deferred, endpoint) {
                if (response && response.data) {
                    ...
                    Alert.error("Error Inesperado, por favor refresca la pagina.");
                    deferred.reject();
                }
            },
                ...
                try {
                    Request({
                        ...
                        if (
                            ...
                            response.data = {message: "No corresponde a la URL del Gateway."};
                            deferred.reject(response);
                        }
                    }, function (response) {
                        if (response.status == 511) {
                            response.data = {message: 'El Gateway requiere autenticacion.'};
                        } else if (response.status == 404) {
                            response.data = {message: "No es posible conectarse al Gateway."};
                        }
                        deferred.reject(response);
                    });
                } catch (err) {
